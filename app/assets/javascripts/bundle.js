...
/***/ function(module, exports, __webpack_require__) {
	...
	Board.prototype.addDot = function (color, pos) {
	  ...
	  var stringPosition = pos[0] + ',' + pos[1];
	  var dotElement = $('<li pos="' + stringPosition + '" class=\'dot ' + color + '\'>');
	...
	Board.prototype.removeDot = function (pos) {
	  ...
	  var newPos = $(pos).attr('pos');
	  this.setDot(undefined, newPos);
	  $(pos).hide('medium', function () {
	    $(pos).remove();
	    _this.resetBoard();
	  });
	};
	...
	function isAdjacentTo(dot) {
	  ...
	  var dotx = $(dot).attr('pos').slice(0, 1);
	  var doty = $(dot).attr('pos').slice(2);
	
	  for (var i = 0; i < candidates.length; i++) {
	
	    var candidatex = $(candidates[i]).attr('pos').slice(0, 1);
	    var candidatey = $(candidates[i]).attr('pos').slice(2);
